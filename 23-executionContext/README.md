...
2. 전역 코드 평가
    1. 전역 실행 컨텍스트 생성 → 콜스택에 `push` = 실행중인 실행 컨텍스트
    2. 전역 렉시컬 환경 생성 → 전역 실행 컨텍스트에 바인딩
        1. 전역 환경 레코드 생성 
            
            *let, const로 생성한 전역변수는 전역 객체의 프로퍼티가 되지 않고 개념적인 블록 내에 존재한다고 함. 
            
            1. 객체 환경 레코드 생성 → `var` 키워드 전역 변수 `x=10`, 함수 선언문 전역함수`first`, 빌트인 전역 프로퍼티/함수/객체
            2. 선언적 환경 레코드 생성 → `let, const` 키워드 전역 변수 `a`
...
3. 전역 코드 실행
    ...
    - 렉시컬 환경에서 검색 → 없으면 외부 렉시컬 환경에 대한 참조가 가리키는 렉시컬 환경으로 가서 검색
    1. 변수가 할당된다. (`let, const` 선언한 전역 변수에 이제 할당된다) `a = 1`
    2. `first`함수가 호출된다.

4. `first`함수 코드 평가
    1. 함수 실행 컨텍스트 생성 → 콜스택에 `push` = 실행중인 실행 컨텍스트
    2. 함수 렉시컬 환경 생성
        1. 함수 환경 레코드 생성 → 매개변수, arguments 객체, 지역변수`y=20`, `b`, 중첩함수`second` 등록
...
5. `first`함수 실행
    1. 할당 `b=2`,`second`함수 실행

6. `second`함수 코드 평가
    1. 함수 실행 컨텍스트 생성 → 콜스택에 `push` = 실행중인 실행 컨텍스트
    2. 함수 렉시컬 환경 생성
        1. 함수 환경 레코드 생성 → 매개변수, arguments 객체, 지역변수`z=30`,`c`, 중첩함수 등록
...
7. `second` 함수 실행
    1. 할당 `c=3`
    2. `console.log(a+b+c+x+y+z+);` 실행 (사실 얘도 콜스택 push/pop과정 있다)
        1. console 식별자 전역에서 검색(BindingObject 통해)
        2. log 메서드 검색: console의 프로토타입 체인으로 검색
        3. a+b+c+x+y+z 평가를 위해 렉시컬 레코드에서 식별자 검색
    3. `second` 함수 실행 컨텍스트 `pop`
  
8. `first` 함수 종료, 실행 컨텍스트 `pop`
9. 전역 코드 실행 종료, 실행 컨텍스트 `pop`
